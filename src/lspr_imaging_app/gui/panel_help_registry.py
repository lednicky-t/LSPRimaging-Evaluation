...
PANEL_HELP: dict[str, PanelHelpEntry] = {
    ...
    "roi_math": PanelHelpEntry(
        title="ROI's math",
        description=(
            "Choose how each ROI pair's masked pixels become the per-wavelength sample and reference values that "
            "everything below (the absorbance spectrum, and in turn the sensorgram) is built from.\n"
            ...
            "10% of pixel values before averaging (a fixed fraction, not user-adjustable) - a middle ground between "
            "Mean and Median. Local background (plane fit) fits a tilted plane to the reference ROI's pixels and "
            "evaluates it at the sample ROI's location instead of using the reference ring's raw mean - this "
            "corrects for a spatial illumination gradient between the sample and reference apertures (useful when "
            "they sit in different rows/columns under uneven illumination); the sample side still uses a plain "
            "mean in this mode.\n"
            "Reduction only re-reads pixels the first time a cube is visited under ANY of these four methods - "
            "every other method's value is computed and cached alongside it for free, so switching Reduction "
            "afterward is instant, same as switching Formula. See ROI's formula (Metric trace section) for how "
            "those two values combine into the final per-wavelength value.\n"
            "Tips:\n"
            "- Reduction applies to every ROI pair and every wavelength - there's no per-ROI override yet.\n"
    ...
    "metric_trace": PanelHelpEntry(
        title="Metric trace",
        description=(
            "Choose which formula produces the absorbance spectrum, then the curve fit, its order, and the "
            "extraction metric used to reduce that spectrum to one value per time point - the sensorgram.\n"
            "Formula: Absorbance (A = -log10(Is/Ir), the default) - Ratio (Is/Ir) - Relative change ((Ir-Is)/Ir) - "
            "mOD absorbance (A in milli-absorbance units). Is/Ir are the sample/reference values from ROI's math's "
            "Reduction. Switching Formula is always instant - it never re-reads pixels, since any formula is "
            "exactly derivable from an already-reduced sample/reference pair (same idea as switching Reduction "
            "itself among its four methods once a cube has been read once - see the ROI's math help entry).\n"
    ...
    "statistics": PanelHelpEntry(
        ...
        description=(
            ...
            "Sensogram display: how the Sensogram plot shows more than one selected ROI. Individual draws each "
            "selected ROI's own trace separately. Average all draws one trace, averaging every selected ROI's own "
            "trace (Mean or Median, with an SD/SEM spread band). Average by group draws one trace per group "
            "present in the selection (see ROI editor grouping) plus one more for any selected ROIs that aren't "
            "in a group. All three only ever combine already-fitted per-ROI sensogram values, never spectra or "
            "pixels - see apps/LSPRi/eva/docs/analysis_pipeline_layers.md. A ROI with no data yet is computed "
    ...
    "spectra_panel": PanelHelpEntry(
        title="Spectra",
        description=(
            "Plots the ROI's formula result (Absorbance by default, or Ratio/Relative change/mOD absorbance, "
            ...
            "result (see ROI's math and ROI's formula).\n"
